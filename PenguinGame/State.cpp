	...
	while (SDL_PollEvent(&event)) {
		...
		if (event.type == SDL_MOUSEBUTTONDOWN) {
			...
			for (int i = objectArray.size() - 1; i >= 0; --i) {
				...
				Vec2 clickedPos((float) mouseX, (float) mouseY);
				if (go->box.Contains(clickedPos)) {
		...
		if (event.type == SDL_KEYDOWN) {
			...
			// Se não, crie um objeto em um angulo aleatorio em um raio 200 da posicao clicada
			else {
				float radAngle = -PI + PI * (rand() % 1001) / 500.0;	// random angle between -PI and PI
				Vec2 randVector = Vec2(200, 0).GetRotated(radAngle);
				Vec2 mouseVector = Vec2(mouseX, mouseY);
...
void State::AddObject(int mouseX, int mouseY) {
	...
	enemyObject->box.y = mouseY - enemySprite->GetWidth()/2;
